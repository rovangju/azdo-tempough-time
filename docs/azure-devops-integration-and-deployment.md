...
3. Run `make package-dev` to create `artifacts/tempough-time-dev.vsix`.
...
require incrementing `version` in `vss-extension.json`, rebuilding the development VSIX, and updating it in the Marketplace.
